...
def resolve_agent_config(user_config: dict | None, session_key: str) -> dict[str, Any]:
    ...
    enabled_val = agent_cfg.get("enabled")
    if has_enabled_key:
        enabled = str(enabled_val).strip().lower() in {"true", "1", "yes", "on"} if isinstance(enabled_val, str) else bool(enabled_val)
    else:
        enabled = role == "soul"
    if role == "standard":
        enabled = False
    ...
    use_memu = agent_cfg.get("use_memu_turn", True)
    out["use_memu_turn"] = str(use_memu).strip().lower() in {"true", "1", "yes", "on"} if isinstance(use_memu, str) else bool(use_memu)
    try:
