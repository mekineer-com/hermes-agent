...
def _bridge_port() -> int:
    ...
    port = whatsapp.get("bridge_port") if isinstance(whatsapp, dict) else None
...
def send_document(
    ...
    *,
    ...
    """POST ``{chatId, filePath, mediaType, caption, fileName}`` to ``/send-media``."""
    chat_id_clean = str(chat_id or "").strip()
    file_path_clean = str(file_path or "").strip()
    if not chat_id_clean or not file_path_clean:
        return False
    import os
    payload: dict = {
        "chatId": chat_id_clean,
        "filePath": file_path_clean,
        "mediaType": "document",
    ...
    url = f"http://127.0.0.1:{_bridge_port()}/send-media"
    req = urllib.request.Request(
        url,
        data=data,
        headers={"Content-Type": "application/json"},
    ...
    try:
        ...
        logger.warning("whatsapp_bridge_client: send_document failed for %s: %s", chat_id_clean, exc)
        return False
