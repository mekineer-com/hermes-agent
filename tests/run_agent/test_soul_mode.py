...
def test_soul_config_not_active_when_disabled(soul_agent):
    soul_agent.configure_soul_mode(enabled=False, role="soul", soul_id="Echo", user_id="test-user", memu_base_url="http://127.0.0.1:8099")
...
def test_build_conversation_id_canonicalizes_whatsapp_gateway_key_dm():
    result = soul_mode.build_conversation_id(
        platform="whatsapp",
        chat_id="12025550188@s.whatsapp.net",
        chat_type="dm",
        gateway_session_key="agent:main:whatsapp:dm:12025550188@s.whatsapp.net",
        canonical_whatsapp_fn=lambda value: {
            "12025550188@s.whatsapp.net": "270699038040215@lid",
...
def test_handle_turn_observes_listen_only_whatsapp_without_public_response(soul_agent, monkeypatch):
    ...
    soul_agent._user_name = "Test User"
...
def test_handle_turn_routes_private_to_self_dm_on_whatsapp(soul_agent, monkeypatch):
    ...
    monkeypatch.setattr(whatsapp_bridge_client, "read_self_dm_jid", lambda: "12025550177@s.whatsapp.net")
    sent = []
    monkeypatch.setattr(
    ...
    assert sent == [("12025550177@s.whatsapp.net", "aside for you")]
    assert result["completed"] is True
...
def test_handle_turn_routes_generic_timeout_crash_to_self_dm_on_whatsapp(soul_agent, monkeypatch):
    soul_agent.platform = "whatsapp"
    soul_agent._chat_id = "120363000000000000@g.us"
    soul_agent._chat_type = "group"
    soul_agent._chat_name = "Familia"
    ...
    mock_client = MagicMock()
    mock_client.memu_turn.side_effect = TimeoutError("timed out")
    soul_agent._soul_config._client = mock_client
    ...
    assert sent == [
        (
            "12025550177@s.whatsapp.net",
            "memU turn failed: TimeoutError: timed out",
        )
    ]
    assert result["completed"] is True
    assert result["turn_exit_reason"] == "soul_mode_memu_error_private_notice"
...
def test_handle_turn_uses_original_user_message_for_memu(soul_agent):
    ...
    soul_agent._soul_config._client = mock_client
    soul_agent.platform = "whatsapp"
    soul_agent._chat_id = "120363@g.us"
    soul_agent._chat_type = "group"
    soul_agent._gateway_session_key = "agent:main:whatsapp:group:120363@g.us:15551234567"

    soul_mode.handle_turn(
        soul_agent, soul_agent._soul_config,
        user_message="[Test User] Echo now has her own desk.",
        conversation_history=[],
        messages=[{"role": "user", "content": "[Test User] Echo now has her own desk."}],
        task_id="test-task",
        original_user_message="Echo now has her own desk.",
        summarize_for_log=lambda x: str(x)[:50],
    )

    mock_client.memu_turn.assert_called_once()
    call_kwargs = mock_client.memu_turn.call_args[1]
    assert call_kwargs["message"] == "Echo now has her own desk."
    assert call_kwargs["user_name"] == str(getattr(soul_agent, "_user_name", "") or "")
